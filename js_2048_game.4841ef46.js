...
class Game {
    ...
    moveLeft() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            for(let col = 0; col < newRow.length - 1; col++)if (newRow[col] !== 0 && newRow[col] === newRow[col + 1]) {
                newRow[col] *= 2;
                newRow[col + 1] = 0;
    ...
    createRandomTile() {
        ...
        if (emptyTiles.length > 0) {
            const { rowIndex, cellIndex } = emptyTiles[Math.floor(Math.random() * emptyTiles.length)];
    ...
    checkStatus() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            if (this.board[r][c] === 0) hasEmptyTile = true;
            if (r < 3 && this.board[r][c] === this.board[r + 1][c] || c < 3 && this.board[r][c] === this.board[r][c + 1]) canMove = true;
        }
        if (!hasEmptyTile && !canMove) this.status = 'lose';
    }
    updateMessage() {
        switch(this.status){
            case 'idle':
